...
{
    public class ApproveTaxiwayChecklistController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Approve(ApprovalParam model)
            {
            var data = await Task.Run(() => Entities.ApproveTaxiwayInspectionChecklist(model.date, model.time, model.approved));
            return Ok(data);
        }
    }
}
